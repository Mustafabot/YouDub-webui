...
try:
    ...
    TTS_AVAILABLE = False
    logger.warning("TTS library not available. Please install with: pip install TTS")
    logger.warning("Voice cloning functionality will be disabled")
...
def load_model(model_path="tts_models/multilingual/multi-dataset/xtts_v2", device='auto'):
    ...
    if not TTS_AVAILABLE:
        logger.warning("TTS not available, cannot load model")
        return
...
def tts(text, output_path, speaker_wav, model_name="tts_models/multilingual/multi-dataset/xtts_v2", device='auto', language='zh-cn'):
    ...
    if not TTS_AVAILABLE:
        logger.warning("TTS not available, skipping TTS generation")
        return

...
def do_everything_wrapper(input_mode, url, local_files, root_folder, num_videos, resolution, translation_target_language, subtitles, auto_upload_video,
    ...
    if not check_network() and input_mode == 'URL 下载':
        ...
        offline_warnings.append("请选择「本地文件」模式或检查网络连接")
...
def demucs_wrapper(folder, model, device, progress, shifts):
...
def whisper_wrapper(folder, model, diarization, download_root, device, batch_size, min_speakers, max_speakers):
    if not folder or not folder.strip():
        return _format_error(
            "未填写文件夹路径",
            ["文件夹路径为空"],
...
def synthesize_wrapper(folder, subtitles, resolution, speed_up, fps):
    if not folder or not folder.strip():
        return _format_error(
            "未填写文件夹路径",
            ["文件夹路径为空"],
    ...
    try:
        result = synthesize_all_video_under_folder(
            folder, subtitles=subtitles, speed_up=float(speed_up), fps=int(fps), resolution=resolution
        )
        return f"✅ {result}" if result and not str(result).startswith("❌") else result
    except Exception as e:
        return _classify_error(e)
...
with gr.Blocks(title='YouDub') as app:
    with gr.Tabs():
        ...
        with gr.Tab('人声分离'):
            ...
            dm_btn.click(
                fn=demucs_wrapper,
                inputs=[dm_folder, dm_model, dm_device, dm_progress, dm_shifts],
                outputs=dm_output
            )
        with gr.Tab('语音识别'):
            ...
            ws_btn.click(
                fn=whisper_wrapper,
                inputs=[ws_folder, ws_model, ws_diarization, ws_download_root, ws_device, ws_batch_size, ws_min_speakers, ws_max_speakers],
                outputs=ws_output
            )
        with gr.Tab('字幕翻译'):
            gr.Markdown("使用大语言模型将字幕翻译为目标语言")
            translation_interface.render()
        with gr.Tab('语音合成'):
        ...
        with gr.Tab('视频合成'):
            ...
            sv_btn.click(
                ...
                inputs=[sv_folder, sv_subtitles, sv_resolution, sv_speed_up, sv_fps],
                outputs=sv_output
            )
        with gr.Tab('上传B站'):
